...
def map_parameters(latitud_orig, longitud_orig, max_dist, minAvg):
	...
	for line in sys.stdin:
		...
		comment = line_list[9].strip()
		avg_nuevo = line_list[3].strip()
		print avg_nuevo
		break
	
		if(is_number(avg_nuevo) and (avg_nuevo > minAvg) and is_number(latitud_nueva) and is_number(longitud_nueva) and comment!=""):
...
minAvg = float(sys.argv[4])


map_parameters(latitud_orig,longitud_orig,max_dist, minAvg)
